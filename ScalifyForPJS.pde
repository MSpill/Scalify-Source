...
void setup() {
  ...
  float hei = screenHeight;
  if (hei > wid) {
    size (max(hei, 700), max((int)(wid), 500), P2D);
  } else {
    size (max(wid, 700), max((int)(hei - 155), 500), P2D);
  }
  smooth(3);
...
void drawEditTuningMenu() {
  ...
  text("Tuning:", width/2.0 - 100 - 23, height/2.0-214);
  float wholeW = 600;
  ...
  for (int i = 0; i < tuning.length; i++) {
    ...
    text (tuning[i], width/2.0 - wholeW/2.0 + gap*i, height/2.0 - 50);
